package commodityCategoryDbService
...
func GetInstance() *CommodityCategoryDbService {
	...
	return &CommodityCategoryDbService{node}
}

type CommodityCategoryDbService struct {
	node *snowflake.Node
}

func (u *CommodityCategoryDbService) Create(supplier *model.CommodityCategory) (*model.CommodityCategory, error) {
...
func (s *CommodityCategoryDbService) Query(user_id int64, page, size int, name *string) ([]model.CommodityCategory, int64, error) {
	if page < 1 {
		page = 1
	}
	if size < 1 || size > 100 {
...
func (s *CommodityCategoryDbService) GetById(user_id, id int64) (model.CommodityCategory, error) {
	var supplier model.CommodityCategory
	err := database.GormDB.
		Where("user_id = ?", user_id).
...
func (c *CommodityCategoryDbService) Update(user_id int64, supplier *model.CommodityCategory) (*model.CommodityCategory, error) {

	_supplier, err := c.GetById(user_id, supplier.ID)
	if err != nil {
		return nil, fmt.Errorf("查询不到该商品类别")
	}
...
func (c *CommodityCategoryDbService) Delete(user_id, id int64) error {
	supplier, err := c.GetById(user_id, id)
	if err != nil {
		return fmt.Errorf("找不到该商品类别")
	}
...
func (u *CommodityCategoryDbService) GenerateSnowflakeID() int64 {
